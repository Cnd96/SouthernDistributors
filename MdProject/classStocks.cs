...
namespace MdProject
{
    class classStocks
    {
        public static DataTable itemsSearch()
        {
            ...
            string query = "Select ib.itemid , ib.batchid,i.itemcategory, i.itemname , ib.quantity,ib.sellingprice,ib.expdate from batch ib inner join item i on ib.itemid=i.itemid  where ib.quantity>0 order by ib.itemid asc";
        ...
        public static DataTable wildCardDescription(string itemcombotext)
        {
            ...
            DataTable db = new DataTable();
            string query = "Select ib.itemid , ib.batchid, i.itemname,i.itemcategory , ib.quantity,ib.sellingprice,ib.expdate from batch ib inner join item i on ib.itemid=i.itemid  where ib.quantity>0 and i.itemname LIKE '%" + itemcombotext + "%' order by ib.itemid asc";
            db = clsConnection.GetData(query);
        ...
        {
            string query = "delete batch where itemid='" + itemid + "' and batchID ='" + bid + "'";
        ...
        {
            string query = "update batch set itemid='" + itemid + "' , batchID ='" + bid + "' , quantity=" + quantity + " , sellingprice=" + sellprice + " , expdate ='" + exp + "' where itemid='" + itemid + "' and batchID ='" + bid + "'";
            int x = clsConnection.SendQuery(query);
